...
- Before a first-time publish (a new `deploy` or `screen publish` — not an `append`/`--update`), ask the user how to handle visibility and access. Ask the two separately, because they are independent layers: visibility controls whether the site shows up in the marketplace, while the access password controls whether viewers can open it.
- Visibility (`--visibility`): for anonymous sessions, default to `public` and tell the user; for authenticated users, ask every time whether to use `public` (marketplace, searchable, likeable) or `unlisted` (link-only, not listed).

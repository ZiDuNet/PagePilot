...
- APP 已连接服务器但还没有归属用户时，只会出现在管理员后台的“待分配设备”；普通用户后台不会显示这类设备，除非输入配对码绑定，或管理员分配给该用户。
...
POST   /api/admin/screens/{screenId}/assign
...
APP 默认通过 `Authorization: Device <token>` 建立 WebSocket；如果升级请求返回 401/403，会自动重试 `deviceToken` 查询参数兜底，用于兼容部分反向代理没有转发升级请求 Authorization 头的情况。设置页会展示脱敏后的实际 WS 地址和最近连接事件，方便排查线上代理配置。
...
python skill/hostctl-deploy/scripts/hostctl_deploy.py screen assign screen_xxx --owner-user-id user_xxx --name "大厅屏"

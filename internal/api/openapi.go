		...
		"paths": map[string]any{
			...
			"/api/deploys": map[string]any{
				"get": map[string]any{
					...
					"responses": map[string]any{"200": map[string]any{"description": "Creation market deploy list", "content": jsonSchemaRef("MarketplaceListResponse")}},
				},
			},
			...
			"/api/deploys/{publicId}": map[string]any{
				"get": map[string]any{
					...
					"responses":  map[string]any{"200": map[string]any{"description": "Deploy metadata", "content": jsonSchemaRef("MarketplaceDeployResponse")}, "404": errorResponse()},
				},
			},
...
func openAPISchemas() map[string]any {
	...
	return map[string]any{
		...
		"DeployResponse": map[string]any{"type": "object", "description": "Deployment result. url and versionUrl are the canonical links selected by appURLMode; pathUrl/domainUrl and versionPathUrl/versionDomainUrl expose every explicitly addressable variant.", "properties": map[string]any{
			"success": boolSchema, "id": str, "code": str,
			"url":              map[string]any{"type": "string", "description": "Canonical application URL selected by the active appURLMode."},
			"pathUrl":          map[string]any{"type": "string", "description": "Path-based application URL. This remains available as a backwards-compatible route in every mode."},
			"domainUrl":        map[string]any{"type": "string", "description": "Application-subdomain URL. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
			"detailUrl":        str,
			"versionUrl":       map[string]any{"type": "string", "description": "Canonical URL for the deployed version selected by the active appURLMode."},
			"versionPathUrl":   map[string]any{"type": "string", "description": "Path-based URL for the deployed version."},
			"versionDomainUrl": map[string]any{"type": "string", "description": "Application-subdomain URL for the deployed version. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
		...
		"VersionCreatedResponse": map[string]any{"type": "object", "description": "Version creation result. URL fields follow the current server app URL configuration, including for historical versions.", "properties": map[string]any{
			"success": boolSchema, "code": str, "versionId": str, "versionNumber": intSchema,
			"url":              map[string]any{"type": "string", "description": "Canonical application URL selected by the active appURLMode."},
			"pathUrl":          map[string]any{"type": "string", "description": "Path-based application URL."},
			"domainUrl":        map[string]any{"type": "string", "description": "Application-subdomain URL. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
			"detailUrl":        str,
			"versionUrl":       map[string]any{"type": "string", "description": "Canonical URL for the newly created version."},
			"versionPathUrl":   map[string]any{"type": "string", "description": "Path-based URL for the newly created version."},
			"versionDomainUrl": map[string]any{"type": "string", "description": "Application-subdomain URL for the newly created version. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
		...
		"ListVersionsResponse": map[string]any{"type": "object", "properties": map[string]any{
			...
			"versions": map[string]any{"type": "array", "items": map[string]any{"$ref": "#/components/schemas/VersionItem"}},
		}},
		"VersionItem": map[string]any{"type": "object", "description": "Historical version metadata. URL fields are resolved from the current app URL configuration when the list is read; domainUrl is omitted when domain serving is unavailable.", "properties": map[string]any{
			"versionNumber": intSchema, "id": str, "title": str, "description": str, "size": intSchema,
			"fileCount": intSchema, "isLocked": boolSchema, "isCurrent": boolSchema, "status": str,
			"createdAt": timeSchema, "templateSourceCode": str, "templateSourceVersion": intSchema,
			"storageBackend": str, "storagePrefix": str,
			"url":       map[string]any{"type": "string", "description": "Canonical historical version URL selected by the active appURLMode."},
			"pathUrl":   map[string]any{"type": "string", "description": "Path-based historical version URL."},
			"domainUrl": map[string]any{"type": "string", "description": "Application-subdomain historical version URL. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
		}},
		...
		"MarketplaceDeployResponse": map[string]any{"type": "object", "description": "Creation market item. url is the canonical application link selected by appURLMode; pathUrl and domainUrl are explicit alternatives.", "properties": map[string]any{
			"id": str, "code": str, "owned": boolSchema, "canManage": boolSchema,
			"currentVersionId": str, "primaryVersionStrategy": str, "title": str, "description": str,
			"filename":  str,
			"url":       map[string]any{"type": "string", "description": "Canonical application URL selected by the active appURLMode."},
			"pathUrl":   map[string]any{"type": "string", "description": "Path-based application URL."},
			"domainUrl": map[string]any{"type": "string", "description": "Application-subdomain URL. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
			"filePath":  str, "fileSize": intSchema, "qrCodePath": str,
			"primaryVersionId": str, "createdAt": timeSchema, "updatedAt": timeSchema,
			"viewCount": intSchema, "likeCount": intSchema, "reuseCount": intSchema,
			"templateSourceCode": str, "templateSourceVersion": intSchema,
			"favoriteCount": intSchema, "favorited": boolSchema, "versionCount": intSchema,
			"expiresAt": timeSchema, "status": str, "visibility": str, "reusePolicy": str,
			"sourceDownloadPolicy": str, "securityMode": str, "category": str,
			"tags": map[string]any{"type": "array", "items": str}, "accessProtected": boolSchema,
			"isPinned": boolSchema, "pinnedAt": timeSchema,
		...
		"MarketplaceListResponse": map[string]any{"type": "object", "description": "Paginated creation market results.", "properties": map[string]any{
			"deploys": map[string]any{"type": "array", "items": map[string]any{"$ref": "#/components/schemas/MarketplaceDeployResponse"}},
		...
		"SiteListItem": map[string]any{"type": "object", "description": "Admin site metadata. url is the canonical link selected by appURLMode; pathUrl and domainUrl are explicit alternatives and are resolved from the current server configuration.", "properties": map[string]any{
			"code": str, "publicId": str, "ownerTokenId": str, "ownerUsername": str,
			"url":            map[string]any{"type": "string", "description": "Canonical application URL selected by the active appURLMode."},
			"pathUrl":        map[string]any{"type": "string", "description": "Path-based application URL, available for compatibility."},
			"domainUrl":      map[string]any{"type": "string", "description": "Application-subdomain URL. Omitted unless domain or dual mode is enabled with an appDomainSuffix."},
			"currentVersion": intSchema, "versionCount": intSchema, "totalSize": intSchema,
			"viewCount": intSchema, "likeCount": intSchema, "reuseCount": intSchema,
			"status": str, "visibility": str, "reusePolicy": str, "sourceDownloadPolicy": str,
			"securityMode": str, "templateSourceCode": str, "templateSourceVersion": intSchema,
			"category": str, "tags": map[string]any{"type": "array", "items": str}, "title": str,
			"description": str, "filename": str, "accessProtected": boolSchema, "isPinned": boolSchema,
			"pinnedAt": timeSchema, "createdAt": timeSchema, "source": str, "lastVersionAt": timeSchema,
		}},
		"SiteListResponse": map[string]any{"type": "object", "properties": map[string]any{
			"success": boolSchema,
			"sites":   map[string]any{"type": "array", "items": map[string]any{"$ref": "#/components/schemas/SiteListItem"}},
		}},
		...
		"SiteDetailResponse": map[string]any{"type": "object", "properties": map[string]any{
			"success":  boolSchema,
			"site":     map[string]any{"$ref": "#/components/schemas/SiteListItem"},
			"versions": map[string]any{"type": "array", "items": map[string]any{"$ref": "#/components/schemas/VersionItem"}},
			"bundle":   map[string]any{"$ref": "#/components/schemas/BundleDetail"},
			"files":    map[string]any{"type": "array", "items": map[string]any{"$ref": "#/components/schemas/ContentFile"}},
			"reuse":    map[string]any{"$ref": "#/components/schemas/ReuseDetail"},

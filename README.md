...
当前代码已经完成 multipart 发布、ZIP 入口识别、Markdown 安全语义渲染、FTS 搜索、Bundle 元数据表和审计日志存储接口等底层能力。但后台审计日志 API/UI、创作市场详情和后台站点详情中的完整文件树 / Bundle 类型 / 安全模式 / 复用参数展示、服务端 Markdown 解析 + highlight.js 高亮 + KaTeX 渲染 + 内置 Mermaid 前端运行时的高级 Markdown 链路、jpage 式模板复用体验、完整运行时视觉 QA 以及真实旧数据库 Docker 升级验证仍未完成。
...
Markdown 会作为一等入口托管，支持相对图片、表格、任务列表、代码块、Mermaid/数学公式语义块和渲染缓存。当前服务端仍以安全渲染为主：Markdown 响应使用更严格的 CSP。后续如果对齐 jpage，应采用服务端 Markdown 解析、highlight.js 高亮、KaTeX `renderToString`，Mermaid 则由平台内置前端运行时配合 nonce 初始化脚本在浏览器端绘图，不应把未验证的外部脚本直接塞进 Markdown 渲染链路。

市场搜索已接入 SQLite FTS5，并保留中文 `LIKE` 回退；老数据库启动时会自动回填索引。新增的渲染缓存、Bundle 元数据和审计日志表都是增量迁移，设计上不会清空已有站点、版本或用户数据；发布前仍需要用一份真实旧数据库跑完整 Docker 升级验证。
...
端到端发布测试需要在 `http://127.0.0.1:8787` 上有本地开发服务器。

...
- 首页支持全屏弹幕动画。
- 手动部署页面位于 `/deploy.html`，创作市场位于 `/market`，屏幕介绍页位于 `/screens/`；API 文档收进后台 `/admin?tab=apiDocs`，机器可读契约位于 `/openapi.json`。
...
- 内置页面 `/deploy.html`、`/market`、`/agents/`、`/screens/` 由 Go 服务内嵌返回；后台 API 文档位于 `/admin?tab=apiDocs`。反向代理应把这些路径原样转发给 PagePilot。
...
环境变量已预留邮箱验证和对象存储入口，后台“运行设置”会展示当前状态：

- `HOSTCTL_EMAIL_VERIFICATION_ENABLED=false`：注册邮箱验证开关。开启后还需要 `HOSTCTL_SMTP_HOST`、`HOSTCTL_SMTP_PORT`、`HOSTCTL_SMTP_USERNAME`、`HOSTCTL_SMTP_PASSWORD`、`HOSTCTL_SMTP_FROM`。
- `HOSTCTL_STORAGE_BACKEND=local|oss`：当前完整文件读写实现是 `local`；`oss` 配置已经进入运行时校验和后台展示，后续接入对象存储适配器后再用于生产文件落盘。
- 阿里云 OSS 相关：`HOSTCTL_OSS_ENDPOINT`、`HOSTCTL_OSS_BUCKET`、`HOSTCTL_OSS_ACCESS_KEY_ID`、`HOSTCTL_OSS_ACCESS_KEY_SECRET`、`HOSTCTL_OSS_PREFIX`、`HOSTCTL_OSS_PUBLIC_BASE_URL`。

# PagePilot
感谢 [Linux DO社区](shturl.cc/yePnlXW1 推广
...
- 阿里云 OSS 相关：`HOSTCTL_OSS_ENDPOINT`、`HOSTCTL_OSS_BUCKET`、`HOSTCTL_OSS_ACCESS_KEY_ID`、`HOSTCTL_OSS_ACCESS_KEY_SECRET`、`HOSTCTL_OSS_PREFIX`、`HOSTCTL_OSS_PUBLIC_BASE_URL`。`HOSTCTL_OSS_ENDPOINT` 建议填写完整地址，例如 `https://oss-cn-hangzhou.aliyuncs.com`；如果省略协议，系统会按 HTTPS 处理。`HOSTCTL_OSS_PREFIX` 建议按环境区分，例如 `prod/pagepilot`。

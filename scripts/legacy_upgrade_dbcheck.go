...
func verify(dbPath string) error {
	...
	checks := []struct {
		...
		{"admin policy and live quota backfilled", `SELECT COUNT(*) FROM admin_users WHERE id = 'user-1' AND username = 'admin' AND is_admin = 1 AND can_like = 1 AND deploy_count = 2`, 1},
		{"anonymous columns and live quota backfilled", `SELECT COUNT(*) FROM anonymous_sessions WHERE id = 'anon-legacy' AND deploy_count = 0 AND agent_id IS NULL AND claimed_by_user_id IS NULL`, 1},
